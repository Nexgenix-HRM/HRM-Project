                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <input
                            ...
                            className="border-none outline-none text-sm font-semibold text-slate-700"
                        />
                    </div>
                </div>
            </div>
                                ...
                                <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            {record.user.profile_image ? (
                                                <img src={`${import.meta.env.VITE_STORAGE_URL}/${record.user.profile_image}`} className="w-10 h-10 rounded-xl object-cover shadow-sm" alt="" />
                                            ) : (
                                            ...
                                            <div>
                                                ...
                                                <div className="text-[10px] font-semibold text-accent uppercase tracking-wider">{record.user.designation || record.user.role}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-600 text-[10px] font-black tracking-wider uppercase border border-emerald-100">
                                    ...
                                    <td className="px-6 py-4">
                                        {record.check_in && record.check_out ? (
                                            <span className="text-[11px] font-black text-slate-400 bg-slate-50 px-2 py-1 rounded-lg">
                                                {calculateDuration(record.check_in, record.check_out)}
                                            </span>
                                        ) : (
                                            <span className="text-[11px] font-black text-rose-400 bg-rose-50 px-2 py-1 rounded-lg flex items-center gap-1">
